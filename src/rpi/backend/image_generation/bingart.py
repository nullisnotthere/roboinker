...
class BingArt:
    ...
    def _get_balance(self):
        ...
        try:
            coins = 15 - int(re.search(r'<div id="reward_c" class="b_hide" data-tb="(\d+)"', response.text).group(1))
    ...
    def generate(self, query, content_type='image'):
        encoded_query = urlencode({'q': query, 'model': 'dalle'})
        print(encoded_query)
        ...
        if content_type == 'image':
            ...
            creation_url = f'{self.base_url}?{encoded_query}&rt={rt}&mdl=0'
