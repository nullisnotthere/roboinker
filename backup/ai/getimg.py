...
generation_data = {
    ...
    "prompt": "A forest in the dark, vibrant neon neural network veins, representing futuristic technology and artificial intelligence. ",  # Your prompt for generating the image
    ...
    try:
        ...
        if response_generation.status_code == 200:
            ...
            if response_upscale.status_code == 200:
                ...
                print(f"Image {i+1} upscaled successfully and saved as '{upscale_file_path}'")
            else:
                # Handle error cases for upscaling
                print(f'Error upscaling image {i+1}: {response_upscale.status_code}')
